...
from segmentation_pytorch.utils.func import per_class_iu, fast_hist
from segmentation_pytorch.utils.serialization import pickle_dump, pickle_load


def evaluate_segmentation( models, test_loader, cfg,
                           fixed_test_size=True,
                           verbose=True):
    device = cfg.GPU_ID
    interp = None
    if fixed_test_size:
        interp = nn.Upsample(size=(cfg.TEST.OUTPUT_SIZE[1], cfg.TEST.OUTPUT_SIZE[0]), mode='bilinear', align_corners=True)
    # eval
...
def load_checkpoint_for_evaluation(model, checkpoint, device):
    ...
    model.load_state_dict(params_trained)
    model.eval()
    model.cuda(device)
